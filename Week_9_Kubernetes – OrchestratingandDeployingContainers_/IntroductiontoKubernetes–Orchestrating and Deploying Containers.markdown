...
3. **Create or Verify the Dockerfile**:
   If your Week 9-A ZIP includes a `Dockerfile`, copy it to `getting-started-app`:
   ```bash
   copy C:\Users\<YourName>\Desktop\week9\Dockerfile .
   ```
   **Command Prompt (single-line):**
   ```bash
   copy C:\Users\<YourName>\Desktop\week9\Dockerfile .
   ```
   If no `Dockerfile` is provided, create one:
   ```bash
   type nul > Dockerfile
   ```
   **Command Prompt (single-line):**
...
4. **Verify and Update package.json**:
   The `Dockerfile` uses `npm start`, so `package.json` must include a `start` script. Check `package.json`:
   ```bash
...
5. **Copy bb.yaml (if provided)**:
   If your Week 9-A ZIP includes `bb.yaml`, copy it:
   ```bash
   copy C:\Users\<YourName>\Desktop\week9\bb.yaml .
   ```
   **Command Prompt (single-line):**
   ```bash
   copy C:\Users\<YourName>\Desktop\week9\bb.yaml .
...
- **“This site can’t be reached” (ERR_CONNECTION_REFUSED)**: If `http://localhost:3000` fails, the container may have crashed. Check:
  ...
  **Command Prompt (single-line):**
  ...
  docker logs <container-id>
  ```
  If you see `npm ERR! Missing script: "start"`, confirm the `start` script in `package.json` and rebuild:
  ```bash
  docker build -t getting-started .
  ```
  **Command Prompt (single-line):**
...
- **Verify `src/index.js` exists**:
  ...
  **Command Prompt (single-line):**
  ...
  dir src
  ```
  Ensure `index.js` is present, as the `start` script uses it.
...
**What We Did**: You cloned the `getting-started` to-do app, created or verified a `Dockerfile` with a health check, updated `package.json` to include a `start` script, built the image, and confirmed it works as a standalone container. This ensures our image is ready for Kubernetes.

## Step 3: Write the Kubernetes Manifest (YAML File)

Now we’ll create a Kubernetes YAML file (`bb.yaml`) to define a **Deployment** (to run the app) and a **Service** (to expose it). If your lab provided `bb.yaml`, you can use it; otherwise, we’ll create it.

1. **Create or Verify bb.yaml**:
   In the `getting-started-app` directory, create `bb.yaml`:
   ```bash
   type nul > bb.yaml
   ```
   **Command Prompt (single-line):**
   ...
   spec:
     ...
     - port: 3000
       ...
       nodePort: 30001
   ```
   If the lab provided `bb.yaml`, verify it matches this content.
...
**What We Did**: You created `bb.yaml` to define a Deployment (running one pod with the `getting-started` app) and a NodePort Service (exposing it on `localhost:30001`). The `imagePullPolicy: Never` ensures Kubernetes uses the local image.
...
Congratulations on deploying your to-do app to Kubernetes! 🎉 You:
- Enabled a local Kubernetes cluster in Docker Desktop.
- Built and tested the `getting-started` app as a Docker container, fixing the `start` script issue in `package.json`.
- Wrote a Kubernetes YAML manifest (`bb.yaml`) for a Deployment and NodePort Service.
- Deployed and accessed the app via `kubectl` and `localhost:30001`.
- Learned about pods, deployments, and services, and how Kubernetes orchestrates containers.

This is a big step into Kubernetes! You’ve seen how it manages containers, making them scalable and resilient. **Next Steps**:
